...
class PathHandler:
    def __init__(self, pred_dir:Path, targ_dir:Path, instance:str):
        """
        Args:
        ...
        self.lp = path.join(pred_dir, instance) # localization prediction
        self.dp = path.join(pred_dir, instance.replace('_pre_', '_post_')) # damage prediction
        self.lt = path.join(targ_dir, instance) # localization target
        self.dt = path.join(targ_dir, instance.replace('_pre_', '_post_')) # damage target
        self.paths = (self.lp, self.dp, self.lt, self.dt)
        
    def load_and_validate_image(self, f_path):
#         assert f_path.is_file(), f"file '{f_path}' does not exist or is not a file"
        img = np.array(Image.open(f_path))
        assert img.dtype == np.uint8, f"{f_path.name} is of wrong format {img.dtype} - should be np.uint8"
        assert set(np.unique(img)) <= {0,1,2,3,4}, f"values must ints 0-4, found {np.unique(img)}, path: {f_path}"
        assert img.shape == (1024,1024), f"{f_path} must be a 1024x1024 image"
        return img
...
class XviewMetrics:
    ...
    def get_path_handlers(self):
        self.path_handlers = []
        for instance in listdir(self.targ_dir):
            if '_pre_' in instance: # localization or damage is fine here
                self.path_handlers.append(PathHandler(self.pred_dir, self.targ_dir, instance))
...
if __name__ == '__main__':
    ...
    pred_dir = 'data/test/predictions'
    targ_dir = 'data/test/masks'
    out_fp = 'data/test/f1_score.json'

    XviewMetrics.compute_score(pred_dir=pred_dir, targ_dir=targ_dir, out_fp=out_fp)