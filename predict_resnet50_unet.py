...
from src.models import ResNet50_Unet_Loc, ResNet50_Unet_Double

from src.utils import *

def predict_loc():
    test_dir = 'data/test/images'
    pred_loc_folder = 'data/test/predict/pred50_loc'
    models_folder = 'weights'

    makedirs(pred_loc_folder, exist_ok=True)

    snap_to_load = '_resnet50_unet_loc_best'

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    model = ResNet50_Unet_Loc(pretrained=True, bilinear=False).to(device)
    ...
    with torch.no_grad():
        for f in tqdm(sorted(listdir(test_dir))):
            if '_pre_' in f:
                ...
                mask = merge_image(splited_imgs=splited_msks)
                cv2.imwrite(path.join(pred_loc_folder, f.replace('.png', '_part1.png')), mask[..., 0], [cv2.IMWRITE_PNG_COMPRESSION, 9])

def predict_cls():
    test_dir = 'data/test/images'
    pred_cls_folder = 'data/test/predict/pred50_cls'
    models_folder = 'weights'

    makedirs(pred_cls_folder, exist_ok=True)

    snap_to_load = '_resnet50_unet_cls_best'
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    model = ResNet50_Unet_Double(pretrained=True, bilinear=False).to(device)
    model = nn.DataParallel(model).to(device)
